...
class Common_model extends CI_Model{
    ...
    public function select_column1($table_name,$select,$where){
		$this->db->select($select);
		$this->db->from($table_name);
		if(!empty($where)){
		    $this->db->where($where);
	...
	public function group_by($select,$table_name,$groupby){
	    $this->db->select($select);
    ...
    function check_unique_res_code($table_name,$id = '', $name) {
 	...
 	public function get_lat_long($address){
		$url = "https://maps.googleapis.com/maps/api/geocode/json?address=".urlencode($address)."&sensor=false&key=AIzaSyAimK0TcCIaX_OagBhfALjpLJcOjQKIRwE";
	    $ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch,CURLOPT_SSL_VERIFYPEER,FALSE);
